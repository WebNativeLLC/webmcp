...
/** Widget served at the site root (Cloudflare Pages `_redirects`). */
const DEFAULT_WIDGET = 'weather'

const SKIP_DIRS = new Set(['dist', 'node_modules'])

function buildRedirects(widgets: string[]): string {
  const lines: string[] = [
    `# Generated by build.ts — do not edit dist/_redirects directly`,
    `/ /${DEFAULT_WIDGET}/ 302`,
  ]

  for (const name of widgets) {
    lines.push(`/${name} /${name}/ 302`)
    lines.push(`/${name}/index.html /${name}/ 301`)
  }

  return `${lines.join('\n')}\n`
}

async function writeRedirects(widgets: string[]): Promise<void> {
  if (!widgets.includes(DEFAULT_WIDGET)) {
    throw new Error(
      `DEFAULT_WIDGET is "${DEFAULT_WIDGET}" but no such widget was built (found: ${widgets.join(', ')})`,
    )
  }

  const redirectsPath = join(outRoot, '_redirects')
  await Bun.write(redirectsPath, buildRedirects(widgets))
  console.log(`  → _redirects`)
  console.log(`      / → /${DEFAULT_WIDGET}/`)
  for (const name of widgets) {
